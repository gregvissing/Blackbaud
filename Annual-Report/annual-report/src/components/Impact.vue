<template>
	<section id="impact" class="cd-section">
		<div class="carousel animateblock" v-bind:class="{ modal: isModal}" ref="impactHeight">
			<div class="carousel-inner">
				<div v-for="(slide, index) in slides" class="carousel-inner-image" :data-active-slide="activeIndex === index" :style="{ backgroundImage: 'url(' + `${slide.image}` + ')' }" v-bind:key="index">
					...
					<button id="show-modal" class="btn modal-read" @click.prevent="openModal(showModal)">READ FULL STORY</button>
					<modal v-if="showModal" @close="showModal = false">
						<button slot="closebutton" class="modal-default-button" @click.prevent="closeModal(showModal)">X</button> 
						<p slot="header" class="title" v-html="slide.modalTitle"></p>
						<h4 slot="subHeader"></h4>                                
						<div slot="body" class="modal-text" v-html="slide.text"></div>
					</modal> 
				</div>
			</div>
		</div>
		<div class="thumbs">
			<div v-for="(thumbnail, index) in thumbnails" class="thumbs-thumbnail" v-bind:key="index">
				<button :data-active-button="activeIndex === index" @click.prevent="update(index)">
...
<script type="text/x-template" id="modal-fundraising-template">
...
export default {
		...
		return {
			...
			slides: [
				{ 
					...
					modalTitle: '<span>REDEFINING <span class="nextTxt">NEXT</span> BY <span class="title-text">INVENTING TOMORROW</span></span>',
				...
				{ 
					...
					modalTitle: '<span>REDEFINING <span class="nextTxt">NEXT</span> BY <span class="title-text">SAVING LIVES</span></span>',
					title: 'SAVING LIVES',
				...
				{ 
					...
					modalTitle: '<span>REDEFINING <span class="nextTxt">NEXT</span> BY <span class="title-text">BENDING THE FUTURE</span></span>',
	...
	methods: {
		...
		computedHeight: function () {
			this.height = this.$refs.impactHeight.clientHeight;
			return this.height;
		}
	},
	mounted() {
	...
	template: '#modal-fundraising-template'
}
</script>
