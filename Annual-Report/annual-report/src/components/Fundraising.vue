<template>
  	<section id="fundraising" class="cd-section fundraising animateblock" ref="appWidth">
		<div class="one-half image" v-bind:style="{ backgroundImage: 'url(' + fundraising.image + ')' }">
		...
		<div class="one-half graphs">
			...
			<div id="odometer" class="odometer">000000000</div>
			<h4>in Total Funds</h4>
			<div class="graph-type-container">                    
				<ul>
					<li v-for="(type, index) in fundraising.types" :class="{ active: fundraising.selected == index }" :key="index"> 
						<span @click="toggle(type)">{{ type.name }}</span> 
					</li>
				</ul>
			</div>
			<div class="chart-container" v-if="fundraising.selected === '0'">
				<template>
					<PurposeChart v-if="width > 767" :width="350" :height="350" />
					<PurposeChart v-if="width <= 767" :width="200" :height="200" />
				</template>
			</div>              
			<div class="chart-container" v-if="fundraising.selected === '1'">
				<template>
					<FundChart v-if="width > 767" :width="350" :height="350" />
					<FundChart v-if="width <= 767" :width="200" :height="200" />
				</template>
				<!-- <fund-chart></fund-chart> -->
			</div>                
			<div class="chart-container" v-if="fundraising.selected === '2'">
				<template>
					<SourceChart v-if="width > 767" :width="350" :height="350" />
					<SourceChart v-if="width <= 767" :width="200" :height="200" />
				</template>
			</div>      
				...
				<vue-tabs>
					<vue-tab label="REVENUES/EXPENSES" :active="true">
						<table>
							...
							<tbody>
								<tr v-for="revenue in revenues" v-bind:class="{ bold: revenue.bold }" :key="revenue">
									<td>{{revenue.title}}</td>
							...
							<tbody>
								<tr v-for="expense in expenses" v-bind:class="{ bold: expense.bold }" :key="expense">
									<td>{{expense.title}}</td>
							...
							<tbody>
								<tr v-for="otherExpense in otherExpenses" v-bind:class="{ bold: otherExpense.bold }" :key="otherExpense">
					...
					<vue-tab label="ASSETS/LIABILITIES">
						<table>
							...
							<tbody>
								<tr v-for="asset in assets" v-bind:class="{ bold: asset.bold }" :key="asset">
									<td>{{asset.title}}</td>
							...
							<tbody>
								<tr v-for="liability in liabilities" v-bind:class="{ bold: liability.bold }" :key="liability">
							...
							<tbody>
								<tr v-for="netAsset in netAssets" v-bind:class="{ bold: netAsset.bold }" :key="netAsset">
